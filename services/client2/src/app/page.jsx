...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen bg-white text-zinc-900 [font-family:Poppins,Manrope,ui-sans-serif,sans-serif]">
        ...
        <div className="mx-auto flex w-full max-w-[1280px] items-center justify-between px-5 py-4 md:px-10">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              Cart
              {cart.length > 0 && (
                <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-zinc-900 px-1.5 text-xs font-bold text-white">{cart.length}</span>
              )}
            </Link>
            <Link href="/login" className="rounded-full border border-zinc-300 px-4 py-2 text-sm font-semibold text-zinc-700 transition hover:border-zinc-400 hover:bg-white">
              Sign In
            </Link>
            <Link href="/shop" className="rounded-full bg-zinc-900 px-5 py-2 text-sm font-semibold text-white transition hover:bg-zinc-700">
              Shop Now
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto grid w-full max-w-[1280px] grid-cols-1 gap-10 px-5 pb-14 pt-10 md:grid-cols-2 md:items-center md:px-8">
        <div className="space-y-7">
          ...
          <h1 className="text-5xl font-normal leading-[0.95] tracking-tight text-zinc-900 md:text-7xl">Bold Looks For The Way You Live.</h1>
          <p className="max-w-xl text-base leading-relaxed text-zinc-600 md:text-lg">
            Discover curated drops, premium essentials, and seasonal edits built for comfort and confidence. ShopPal blends trend and utility in one clean experience.
          </p>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Link href="/signup" className="rounded-full border border-zinc-300 bg-white px-6 py-3 text-sm font-bold text-zinc-700 transition hover:border-zinc-500">
              Join ShopPal
            </Link>
          </div>
          <div className="grid max-w-md grid-cols-3 gap-3 pt-1 text-center">
            <div className="rounded-2xl border border-zinc-200 bg-white p-3">
              ...
              <p className="text-xs uppercase tracking-wide text-zinc-500">Happy Buyers</p>
            </div>
            <div className="rounded-2xl border border-zinc-200 bg-white p-3">
              <p className="text-xl font-black">350+</p>
              <p className="text-xs uppercase tracking-wide text-zinc-500">New Arrivals</p>
            </div>
            <div className="rounded-2xl border border-zinc-200 bg-white p-3">
              <p className="text-xl font-black">24h</p>
              <p className="text-xs uppercase tracking-wide text-zinc-500">Dispatch</p>
            </div>
          </div>
        </div>
        <div className="relative mx-auto w-full max-w-xl">
          ...
          <div className="relative overflow-hidden rounded-[2.25rem] border border-zinc-200 bg-white p-3 shadow-[0_24px_60px_-28px_rgba(0,0,0,0.35)]">
            <Image src={HeroImg} alt="Fashion Hero" width={1000} height={1100} priority className="h-[560px] w-full rounded-[1.8rem] object-contain" />
            <div className="absolute bottom-8 left-8 rounded-2xl bg-white/90 px-4 py-3 shadow-md backdrop-blur">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">New Drop</p>
      ...
      <section className="mx-auto w-full max-w-[1280px] px-5 py-12 md:px-10">
        <div className="mb-8 flex items-end justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-zinc-500">Shop by Category</p>
            <h2 className="mt-2 text-3xl font-black md:text-4xl">Find your signature fit</h2>
          </div>
          <Link href="/shop" className="hidden text-sm font-bold text-zinc-600 transition hover:text-zinc-900 md:inline-flex">
            View all categories
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((cat) => (
            <Link key={cat.name} href={cat.href} className="group relative overflow-hidden rounded-3xl border border-zinc-200 bg-white">
              <Image src={cat.img} alt={cat.name} width={600} height={740} className="h-72 w-full object-cover transition duration-500 group-hover:scale-105" />
      ...
      <section className="mx-auto w-full max-w-[1280px] px-5 py-14 md:px-10">
        <div className="mb-8 flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-zinc-500">New Arrivals</p>
            <h2 className="mt-2 text-3xl font-black md:text-4xl">Fresh picks this week</h2>
          </div>
          <Link
            href="/shop"
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {products.slice(0, 8).map((item) => (
            <article key={item.id} className="group rounded-3xl border border-zinc-200 bg-white p-3 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
              <div className="overflow-hidden rounded-2xl">
                <Image src={item.image} alt={item.name} width={500} height={600} className="h-64 w-full object-cover transition duration-500 group-hover:scale-105" />
              </div>
              <div className="space-y-2 p-2 pt-4">
                <h3 className="line-clamp-1 text-lg font-black text-zinc-900">{item.name}</h3>
                <p className="line-clamp-2 text-sm text-zinc-500">{item.description}</p>
                <div className="flex items-center justify-between pt-1">
                  <p className="text-base font-extrabold text-zinc-900">${item.price.toFixed(2)}</p>
                  <button
      ...
      <section className="mx-auto w-full max-w-[1280px] px-5 pb-16 pt-4 md:px-10">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <article className="rounded-3xl border border-zinc-200 bg-white p-7">
            ...
            <p className="mt-2 text-sm leading-relaxed text-zinc-600">Same-day dispatch on most orders, with end-to-end tracking from checkout to doorstep.</p>
          </article>
          <article className="rounded-3xl border border-zinc-200 bg-white p-7">
            <Sparkles className="mb-4 h-7 w-7 text-[#f2875b]" />
            <h3 className="text-xl font-black">Curated Trends</h3>
            <p className="mt-2 text-sm leading-relaxed text-zinc-600">Handpicked edits built around fit, quality, and wearable style.</p>
          </article>
          <article className="rounded-3xl border border-zinc-200 bg-white p-7">
            <RefreshCcw className="mb-4 h-7 w-7 text-[#f2875b]" />
            <h3 className="text-xl font-black">Easy Returns</h3>
            <p className="mt-2 text-sm leading-relaxed text-zinc-600">Hassle-free returns and exchange support when the fit is not quite right.</p>
          </article>
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-[1280px] px-5 pb-14 md:px-10">
        <div className="relative overflow-hidden rounded-[2rem] bg-zinc-900 px-6 py-12 text-center text-zinc-100 md:px-12">
          ...
          <h2 className="text-3xl font-black md:text-4xl">Build your next signature outfit</h2>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-zinc-300 md:text-base">
            Join a growing community of style-first shoppers and unlock early access to exclusive drops and member-only offers.
          </p>
          <Link
            href="/shop"
